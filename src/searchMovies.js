...
export class SearchMovies extends Component {
  ...
  constructor() {
    super();
    this.state = {movieResult: [], searchKeyword: '', resultFound: true, Loading: true};
  }
  ...
  runAPI = () => {
    ...
    .then((res) => {
        this.setState({movieResult: res.data.results, Loading: false,resultFound: true})
    }).catch((err) => {
        console.log(err);
        this.setState({Loading: false,resultFound: false})
    })
  }
    ...
    return (
      <>
        <div id="container">
            ...
            {
                this.state.Loading ?  'Loding...' :  <div id="result">
                        {
